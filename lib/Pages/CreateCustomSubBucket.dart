...
class CreateCustomsubBucket extends StatefulWidget {
  CreateCustomsubBucket({Key key, this.id}) : super(key: key);
  final id;

  @override
  _CreateCustomsubBucketState createState() => _CreateCustomsubBucketState();
}

class _CreateCustomsubBucketState extends State<CreateCustomsubBucket> {
  @override
  void initState() {
    final BlockCustomSubbucket _provider =
        Provider.of<BlockCustomSubbucket>(context, listen: false);
    apiCalling(_provider);
    FireBaseAnalyticsServices.setCurrentScreen(
        screenClass: 'Default Task', screenName: 'Default Task class');
    super.initState();
  }

  apiCalling(BlockCustomSubbucket provider) async {
    await ApiUtils.getSubtoSubCustomBucketTaskData(
        provider: provider, id: widget.id);
  }

  @override
  Widget build(BuildContext context) {
    final BlockCustomSubbucket _provider = Provider.of<BlockCustomSubbucket>(
      context,
    );
                                          ...
                                          children: [
                                            ...
                                            InkWell(
                                              ...
                                              onTap: () {
                                                ...
                                                ApiUtils.sendSubBucketTaskData(
                                                    provider: _provider,
                                                    scaffoldkey:
